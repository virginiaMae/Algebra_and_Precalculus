import { R as React, bx as useDoenetRenderer, r as reactExports, bK as deepCompare, j as jsx, F as Fragment } from "./PageViewer-d914b069.js";
...
const legend = React.memo(function Legend(props) {
  ...
  let swatches = reactExports.useRef([]);
  let labels = reactExports.useRef([]);
  let previousElements = reactExports.useRef(null);
  let previousPosition = reactExports.useRef(null);
  let previousLimits = reactExports.useRef(null);
  ...
  if (board) {
    if (!deepCompare(previousElements.current, SVs.legendElements) || !deepCompare(previousLimits.current, SVs.graphLimits) || previousPosition.current !== SVs.position) {
