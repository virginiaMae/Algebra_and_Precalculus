...
const polygon = React.memo(function Polygon(props) {
  ...
  let polygonJXG = reactExports.useRef(null);
  ...
  function initializePoints(polygon2) {
  ...
  function dragHandler(i, e) {
    ...
    if (!viaPointer || Math.abs(e.x - pointerAtDown.current[0]) > 0.1 || Math.abs(e.y - pointerAtDown.current[1]) > 0.1) {
      ...
      if (i === -1) {
        ...
        for (let i2 = 0; i2 < polygonJXG.current.vertices.length - 1; i2++) {
          if (viaPointer) {
            ...
            let vertex = polygonJXG.current.vertices[i2];
  ...
  if (board) {
    if (!polygonJXG.current) {
      ...
      if (SVs.numVertices > previousNumVertices.current) {
        ...
        for (let i = previousNumVertices.current - 1; i >= SVs.numVertices; i--) {
