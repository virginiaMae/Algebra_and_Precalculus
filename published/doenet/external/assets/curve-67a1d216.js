...
import { BoardContext, LINE_LAYER_OFFSET, VERTEX_LAYER_OFFSET, CONTROL_POINT_LAYER_OFFSET } from "./graph-8a8db909.js";
import "./css-5483d03f.js";
import "./visibility-sensor-57589aaf.js";
const curve = React.memo(function Curve(props) {
  ...
  let throughPointsJXG = reactExports.useRef(null);
  let controlPointsJXG = reactExports.useRef(null);
  let previousCurveType = reactExports.useRef(null);
  let draggedControlPoint = reactExports.useRef(null);
  let draggedThroughPoint = reactExports.useRef(null);
  ...
  let previousFlipFunction = reactExports.useRef(null);
  let segmentAttributes = reactExports.useRef(null);
  let throughPointAttributes = reactExports.useRef(null);
  let throughPointAlwaysVisible = reactExports.useRef(null);
  let throughPointHoverVisible = reactExports.useRef(null);
  let controlPointAttributes = reactExports.useRef(null);
  let previousNumberOfPoints = reactExports.useRef(null);
  let segmentsJXG = reactExports.useRef([]);
  let vectorControlsVisible = reactExports.useRef(null);
  let hitObject = reactExports.useRef(null);
  let vectorControlDirections = reactExports.useRef(null);
  let previousVectorControlDirections = reactExports.useRef(null);
  let fixed = reactExports.useRef(false);
  let fixLocation = reactExports.useRef(false);
  let switchable = reactExports.useRef(false);
  let tpCoords = reactExports.useRef([]);
  let cvCoords = reactExports.useRef([]);
  fixed.current = SVs.fixed;
  fixLocation.current = !SVs.draggable || SVs.fixLocation || SVs.fixed;
  switchable.current = SVs.switchable && !SVs.fixed;
  vectorControlDirections.current = SVs.vectorControlDirections;
  let lastThroughPointPositionsFromCore = reactExports.useRef(null);
  lastThroughPointPositionsFromCore.current = SVs.numericalThroughPoints;
  let lastControlPointPositionsFromCore = reactExports.useRef(null);
  ...
  function deleteControls() {
    if (segmentsJXG.current.length > 0) {
      ...
      segmentsJXG.current = [];
      controlPointsJXG.current.forEach(
        (x) => x.forEach((y) => {
          if (y) {
            y.off("drag");
            y.off("down");
  ...
  function makeVectorControlVisible(i) {
    if (!SVs.hiddenControls[i]) {
      if (controlPointsJXG.current[i][0]) {
        let isVisible = (i > 0 || SVs.extrapolateBackward) && ["symmetric", "both", "previous"].includes(
      ...
      if (controlPointsJXG.current[i][1]) {
        let isVisible = (i < throughPointsJXG.current.length - 1 || SVs.extrapolateForward) && ["symmetric", "both", "next"].includes(
          vectorControlDirections.current[i]
        );
