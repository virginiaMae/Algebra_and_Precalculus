import { aB as NodeProp, aC as Tree, aD as TreeFragment, aE as Parser, aF as NodeType, aG as ContextTracker, aH as ExternalTokenizer, aI as LRParser, aA as Recoil_index_8, aJ as parser, t as Recoil_index_20, r as reactExports, j as jsx, F as Fragment } from "./PageViewer-d914b069.js";
...
class EditorSelection {
  ...
  */
  static range(anchor, head, goalColumn) {
    ...
    return head < anchor ? new SelectionRange(
      head,
      anchor,
      16 | goal | 8
      /* AssocAfter */
...
const tooltipConfig = /* @__PURE__ */ Facet.define({
  combine: (values) => {
    var _a2, _b, _c;
    return {
      position: ios ? "absolute" : ((_a2 = values.find((conf) => conf.position)) === null || _a2 === void 0 ? void 0 : _a2.position) || "fixed",
...
const StartTag = 1, StartCloseTag = 2, MissingCloseTag = 3, mismatchedStartCloseTag = 4, incompleteStartCloseTag = 5, commentContent$1 = 36, piContent$1 = 37, cdataContent$1 = 38, Element$1 = 10, OpenTag = 12;
...
function completeFromSchema(eltSpecs, attrSpecs) {
  ...
  return (cx) => {
    ...
    if (type == "openTag") {
      ...
      let parentName = elementName(doc2, context);
      if (parentName) {
      ...
      let parentName = elementName(doc2, context), parent = byName[parentName];
...
const editorConfigStateAtom = Recoil_index_8({
...
function CodeMirror({
  ...
  let editorConfig = Recoil_index_20(editorConfigStateAtom);
  let view = reactExports.useRef(null);
  let parent = reactExports.useRef(null);
  const [count, setCount] = reactExports.useState(0);
  const changeFunc = reactExports.useCallback((tr) => {
    if (tr.docChanged) {
  ...
  if (readOnly2 && ((_b = (_a2 = view.current) == null ? void 0 : _a2.state) == null ? void 0 : _b.facet(EditorView.editable))) {
  ...
  const tabCommand = ({ state: state2, dispatch }) => {
    dispatch(
      state2.update(state2.replaceSelection(tab), {
        scrollIntoView: true,
  ...
  const copyCommand = ({ state: state2, dispatch }) => {
    if (state2.selection.main.empty) {
      selectLine({ state: state2, dispatch });
  ...
  const cutCommand = ({ state: state2, dispatch }) => {
    if (state2.selection.main.empty) {
      selectLine({ state: state2, dispatch });
      document.execCommand("copy");
      if (state2.doc.lineAt(state2.selection.main.from).number !== state2.doc.lines) {
      ...
      document.execCommand("copy");
      dispatch(
        state2.update(state2.replaceSelection(""), {
          scrollIntoView: true,
          annotations: Transaction.userEvent.of("input")
        })
      );
  ...
  const doenetExtensions = reactExports.useMemo(
    () => [
      basicSetup,
      doenet(doenetSchema),
  ...
  const matchTag = reactExports.useCallback(
    (tr) => {
      ...
      if (tr.annotation(Transaction.userEvent) == "input" && tr.newDoc.sliceString(cursorPos - 1, cursorPos) === ">") {
        ...
        let tagName2 = tr.newDoc.sliceString(tagNameNode.from, tagNameNode.to);
        let tra = tr.state.update({
          changes: { from: cursorPos, insert: "</".concat(tagName2, ">") },
          sequential: true
        });
        changeFunc(tra);
        return [
  ...
  reactExports.useEffect(() => {
    if (view.current === null && parent.current !== null) {
      ...
      if (readOnly2 && view.current.state.facet(EditorView.editable)) {
        setCount((old) => {
          return old + 1;
        });
      }
  ...
  reactExports.useEffect(() => {
    if (view.current !== null && parent.current !== null) {
      ...
      } else if (!readOnly2 && !view.current.state.facet(EditorView.editable)) {
        view.current.dispatch({
  ...
  return /* @__PURE__ */ jsx(Fragment, { children: /* @__PURE__ */ jsx("div", { ref: parent, style: divStyle }) });
}
const doenetSchema = {
...
let parserWithMetadata = parser.configure({
  props: [
    ...
    styleTags({
      ...
      MismatchedCloseTag: tags.invalid,
      "StartTag StartCloseTag EndTag SelfCloseEndTag": tags.angleBracket,
      "MismatchedCloseTag/TagName": [tags.tagName, tags.invalid],
      "MismatchedCloseTag/StartCloseTag": tags.invalid,
...
const doenet = (conf = {}) => new LanguageSupport(
  doenetLanguage,
